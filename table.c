...
int main()
{
    ...
    int shmid1 = shmget(TEST + table_no, SHARED_MEMORY_SIZE, IPC_CREAT | 0777);
    if (shmid1 == -1)
    {
    ...
    int shmid2 = shmget(HOTEL_KEY + table_no, SHARED_MEMORY_SIZE, IPC_CREAT | 0777);
    if (shmid2 == -1)
    {
        perror("shmget failed");
        exit(EXIT_FAILURE);
    ...
    struct shared_data *shared_data = shmat(shmid1, NULL, 0);
    ...
    while (1)
    {
        ...
        if(shared_data->valid == 2) // in progress
        {
            continue;
        }
        ...
        { // failed
            printf("Order taking invaild try again\n");
            create_customer(cust, shared_data);
            shared_data->valid = 2; // in progress
        }
        else if(shared_data->valid == 1) // new set of cust
        {
            printf("The total bill amount is %d INR.\n", shared_data->billing_amt);
            shared_data->valid=3;
            if(shared_data_hotel_manager->termination_of_table){
                shared_data->termination_of_waiter=1;
                shared_data->valid=2;
                break;
            }
